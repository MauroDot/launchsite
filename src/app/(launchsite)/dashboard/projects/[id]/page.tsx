...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ edit?: string }> }) {
  ...
  try { [project, isAdmin] = await Promise.all([getProject(id), requireUser().then((user) => user.role === "ADMIN")]); } catch (error) { if (error instanceof Error && error.message === "NOT_FOUND") notFound(); console.error("Unable to load website project", { projectId: id, error }); }
  ...
  if (!project.isDemo && !project.isUnassignedSample) {
