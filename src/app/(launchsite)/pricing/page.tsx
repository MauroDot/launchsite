...
export default async function PricingPage({ searchParams }: { searchParams: Promise<{ checkout?: string }> }) {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
    ...
    <div className="mt-10 grid gap-6 lg:grid-cols-3">{(Object.keys(plans) as Plan[]).map((key) => {
      ...
      const pending = user && entitlements.pendingPlan === key;
      return <article key={key} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
        ...
        {key === "FREE" ? <Link className="inline-block rounded-full border px-5 py-3 text-center text-sm font-semibold" href={user ? "/dashboard" : "/signup"}>{user ? "Go to your websites" : "Start building"}</Link> : !user ? <Link className="inline-block rounded-full bg-slate-900 px-5 py-3 text-center text-sm font-semibold text-white" href="/login?callbackUrl=/pricing">Sign in to choose {plan.name}</Link> : current ? <span className="rounded-full border border-lime-300 px-5 py-3 text-center text-sm font-semibold text-lime-900">Current plan</span> : pending ? <span className="rounded-full border border-amber-300 px-5 py-3 text-center text-sm font-semibold text-amber-900">Scheduled change</span> : manage && entitlements.plan === "STARTER" && key === "BUSINESS" ? <div className="space-y-3"><PlanChangeButton target="BUSINESS">Upgrade to Business</PlanChangeButton><BillingButton>Manage billing</BillingButton></div> : manage && entitlements.plan === "BUSINESS" && key === "STARTER" ? <div className="space-y-3"><PlanChangeButton target="STARTER">Downgrade to Starter</PlanChangeButton><BillingButton>Manage billing</BillingButton></div> : manage ? <BillingButton>Manage billing</BillingButton> : <BillingButton plan={key}>Choose {plan.name}</BillingButton>}
      </article>;
    })}</div>
