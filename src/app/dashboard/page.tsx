...
export default async function DashboardPage() {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12 sm:px-8"><div className="flex items-end justify-between"><div><p className="eyebrow">Workspace</p><h1 className="mt-4 text-4xl font-semibold">Your websites</h1></div><Link className="rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white" href="/create">Create a website</Link></div>{projects.length === 0 ? <p className="mt-10 text-slate-600">No saved projects yet.</p> : <div className="mt-10 grid gap-4 md:grid-cols-2">{projects.map((project) => <Link className="rounded-2xl border border-slate-200 p-6" href={`/dashboard/projects/${project.id}`} key={project.id}><h2 className="text-xl font-semibold">{project.business.businessName}</h2><p className="mt-1 text-sm text-slate-600">{project.business.category}</p><p className="mt-7 text-sm">{project.visualStyle} · {project.status.toLowerCase()} · Updated {project.updatedAt.toLocaleDateString()}</p></Link>)}</div>}</section>;
}
