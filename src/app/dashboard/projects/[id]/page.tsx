...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) notFound();
  return <><div className="mx-auto max-w-6xl px-5 pt-8 sm:px-8"><Link className="text-sm font-semibold text-slate-600" href="/dashboard">← All projects</Link><h1 className="mt-5 text-3xl font-semibold">{project.business.businessName}</h1><div className="mt-5"><GenerateContentButton hasGeneratedContent={Boolean(project.generatedContent)} projectId={project.id} /></div></div><CreateWizard initialProject={project} /></>;
}
