...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ edit?: string }> }) {
  ...
  if (!project) notFound();
  return <><div className="mx-auto max-w-6xl px-5 pt-8 sm:px-8"><Link className="text-sm font-semibold text-slate-600" href="/dashboard">← All projects</Link><h1 className="mt-5 text-3xl font-semibold">{project.business.businessName}</h1><div className="mt-5 flex flex-wrap items-start gap-3"><Link className="rounded-full border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-800" href={`/dashboard/projects/${project.id}/preview`}>Preview website</Link><Link className="rounded-full border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-800" href={`/dashboard/projects/${project.id}?edit=1`}>Edit business facts</Link><GenerateContentButton hasGeneratedContent={Boolean(project.generatedContent)} projectId={project.id} /></div></div><CreateWizard initialProject={project} initialStep={edit === "1" ? 0 : 7} /></>;
}
