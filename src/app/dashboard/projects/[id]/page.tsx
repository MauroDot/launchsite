...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ edit?: string }> }) {
  const { id } = await params; const { edit } = await searchParams; let project: Awaited<ReturnType<typeof getProject>> | undefined;
  try { project = await getProject(id); } catch (error) { console.error("Unable to load website project", error); }
