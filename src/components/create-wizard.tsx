...
import { brandTones, callToActions, initialBusinessProfile, visualStyles, type BusinessProfile, type PersistedWebsiteProject, type VisualStyle } from "@/lib/website-types";
...
export function CreateWizard({ initialProject }: { initialProject?: PersistedWebsiteProject }) {
  const router = useRouter();
  const [step, setStep] = useState(initialProject ? 3 : 0);
  const [business, setBusiness] = useState<BusinessProfile>(initialProject?.business ?? initialBusinessProfile);
  const [visualStyle, setVisualStyle] = useState<VisualStyle>(initialProject?.visualStyle ?? "Modern");
  ...
  async function save() {
    setSaving(true);
    setSaveError(null);
    const result = await saveProjectAction({ business, visualStyle }, initialProject?.id);
    setSaving(false);
  ...
  return <section className="bg-slate-50 py-10 sm:py-14"><div className="mx-auto max-w-6xl px-5 sm:px-8">
    ...
    <div className={step === 3 ? "mt-0" : "mx-auto mt-10 max-w-3xl"}>
      ...
      {step === 3 && <div className="mx-auto mt-5 flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-600">{initialProject ? "Update your details, then save your changes." : "Happy with this direction? Save it to your dashboard."}</p><button className="w-fit rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white disabled:opacity-60" disabled={saving} onClick={save} type="button">{saving ? "Saving…" : initialProject ? "Save changes" : "Save project"}</button>{saveError && <p className="text-sm font-medium text-rose-700">{saveError}</p>}</div>}
    </div></div></section>;
}
