...
async function resolveProject(slug: string | null, host: string | null) {
  ...
  if (custom) {
    const domain = await prisma.domain.findUnique({ where: { hostname }, select: { status: true, project: { select: { id: true, publicSlug: true, isPublished: true, businessName: true, email: true } } } });
  ...
  return prisma.websiteProject.findFirst({ where: { publicSlug: slug, isPublished: true }, select: { id: true, publicSlug: true, isPublished: true, businessName: true, email: true } });
}
...
export async function createPublicLead(payload: unknown, host: string | null) {
  ...
  try { await notifyLead({ leadId: lead.id, projectId: lead.projectId, recipient: project.email, businessName: project.businessName, name: input.name, email: input.email, phone: input.phone, message: input.message }); console.info("Lead notification sent", { leadId: lead.id, projectId: lead.projectId, timestamp: new Date().toISOString() }); }
  catch (error) { console.error("Lead notification failed", { leadId: lead.id, projectId: lead.projectId, timestamp: new Date().toISOString(), errorType: error instanceof Error ? error.name : "Unknown" }); }
  return lead;
}
