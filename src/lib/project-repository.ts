...
export async function getPublicProject(publicSlug: string) {
  ...
  if (!slug) return null;
  const record = await prisma.websiteProject.findFirst({ where: { publicSlug: slug, isPublished: true }, include: { domain: true, services: { orderBy: { position: "asc" } }, workSamples: { orderBy: { position: "asc" } }, testimonials: { orderBy: { position: "asc" } } } });
  return record ? toPublicSite(toProject(record)) : null;
}

export async function getPublicProjectByDomain(hostname: string) {
  const record = await prisma.websiteProject.findFirst({ where: { isPublished: true, domain: { is: { hostname, status: "ACTIVE" } } }, include: { domain: true, services: { orderBy: { position: "asc" } }, workSamples: { orderBy: { position: "asc" } }, testimonials: { orderBy: { position: "asc" } } } });
  return record ? toPublicSite(toProject(record)) : null;
}
