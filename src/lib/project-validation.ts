import { brandTones, callToActions, visualStyles, type WebsiteProjectInput } from "@/lib/website-types";

export type ProjectValidationResult = { valid: true; services: string[] } | { valid: false; message: string };

export function validateProjectInput(input: WebsiteProjectInput): ProjectValidationResult {
  const { business } = input;
  const services = business.services.split(/[\n,]/).map((service) => service.trim()).filter(Boolean).slice(0, 12);
